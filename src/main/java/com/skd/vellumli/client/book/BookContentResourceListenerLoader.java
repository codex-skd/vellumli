...
public class BookContentResourceListenerLoader extends SimpleJsonResourceReloadListener<JsonElement>
	...
	public static final Identifier ID = Identifier.fromNamespaceAndPath(VellumliAPI.MOD_ID, "resource_pack_books");
	...
	private BookContentResourceListenerLoader() {
		super(ExtraCodecs.JSON, FileToIdConverter.json("vellumli_books"));
	}
	...
	protected void apply(Map<Identifier, JsonElement> map, ResourceManager manager, ProfilerFiller profiler) {
		...
		for (var entry : map.entrySet()) {
			...
			if (!matcher.matches()) {
				VellumliAPI.LOGGER.trace("Ignored file {}", key);
				continue;
			}
		...
		VellumliAPI.LOGGER.info("{} preloaded {} jsons", getClass().getSimpleName(), count);
		this.data = data;
	}
	...
	public void findFiles(Book book, String dir, List<Identifier> list) {
		...
		VellumliAPI.LOGGER.info("{}: Files found in {}", getClass().getSimpleName(), stopwatch.stop());
	}

	@Nullable
	@Override
	public LoadResult loadJson(Book book, Identifier file) {
		VellumliAPI.LOGGER.trace("Loading {}", file);
		var map = data.get(book.id);
		if (map == null) {
			return null;
		}
		String path = file.getPath();
		// Drop vellumli_books/ and json suffix
