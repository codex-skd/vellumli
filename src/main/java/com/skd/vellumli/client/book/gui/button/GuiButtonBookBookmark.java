...
public class GuiButtonBookBookmark extends GuiButtonBook {
	...
	protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (bookmark != null && entry != null) {
			...
			if (multiblock) {
				s = I18n.get("vellumli.gui.lexicon.visualize_letter");
			}
	...
	private static Component[] getTooltip(Book book, @Nullable Bookmark bookmark, boolean multiblock) {
		if (bookmark == null) {
			return new Component[] { Component.translatable("vellumli.gui.lexicon.add_bookmark") };
		}
		...
		return new Component[] {
				...
				Component.translatable(multiblock
						? "vellumli.gui.lexicon.multiblock_bookmark"
						: "vellumli.gui.lexicon.remove_bookmark").withStyle(ChatFormatting.GRAY)
		};
	}
