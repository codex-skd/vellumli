...
import com.skd.vellumli.client.book.BookCategory;
import com.skd.vellumli.client.book.BookEntry;
import com.skd.vellumli.client.book.gui.button.GuiButtonBook;
import com.skd.vellumli.client.book.gui.button.GuiButtonBookResize;
...
import static com.skd.vellumli.client.book.gui.GuiBookEntryList.ENTRIES_IN_FIRST_PAGE;
...
public class GuiBookLanding extends GuiBook {
	...
	public void init() {
		...
		addRenderableWidget(new GuiButtonBook(this, x + (pos++) * dist, y, 330, 31, 11, 11, this::handleButtonHistory,
				Component.translatable("vellumli.gui.lexicon.button.history")));

		// Advancements tab button
		// VELLUMLI NOTE (deferred in this beta): Patchouli's GuiAdvancementsExt screen is not
		// ported yet, so the advancements tab button is not shown for the time being.
		if (false && book.advancementsTab != null) {
			addRenderableWidget(new GuiButtonBook(this, x + (pos++) * dist, y, 330, 20, 11, 11, this::handleButtonAdvancements,
					Component.translatable("vellumli.gui.lexicon.button.advancements")));
		}

		Player player = Minecraft.getInstance().player;
		if (player != null && player.isCreative()) {
			addRenderableWidget(new GuiButtonBook(this, x + (pos++) * dist, y, 308, 9, 11, 11, this::handleButtonEdit,
					Component.translatable("vellumli.gui.lexicon.button.editor"),
					Component.translatable("vellumli.gui.lexicon.button.editor.info").withStyle(ChatFormatting.GRAY)));
		}
	...
	private void addCategoryButton(int i, @Nullable BookCategory category) {
		...
		if (category == null) {
			addRenderableWidget(new GuiButtonCategory(this, x, y, book.getIcon(), Component.translatable("vellumli.gui.lexicon.index"), this::handleButtonIndex));
		} else {
	...
	void drawForegroundElements(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (book.getContents().pamphletCategory == null) {
			drawCenteredStringNoShadow(graphics, I18n.get("vellumli.gui.lexicon.categories"), RIGHT_PAGE_X + PAGE_WIDTH / 2, TOP_PADDING, book.headerColor);
		...
		if (book.getContents().isErrored()) {
			...
			drawCenteredStringNoShadow(graphics, I18n.get("vellumli.gui.lexicon.loading_error"), x, y, 0xFF0000);
			drawCenteredStringNoShadow(graphics, I18n.get("vellumli.gui.lexicon.loading_error_hover"), x, y + 10, 0x777777);
	...
	private void makeErrorTooltip() {
		...
		if (!lines.isEmpty()) {
			lines.add(Component.translatable("vellumli.gui.lexicon.loading_error_log").withStyle(ChatFormatting.GREEN));
			setTooltip(lines);
		}
	}
	...
	private void handleButtonAdvancements(Button button) {
		// VELLUMLI NOTE (deferred in this beta): GuiAdvancementsExt is not ported yet.
		// This button is currently not added by init(); kept for source parity with Patchouli.
	}
	...
	private void handleButtonEdit(Button button) {
		if (minecraft.hasShiftDown()) {
			...
			if (minecraft.player != null) {
				minecraft.player.sendSystemMessage(Component.translatable("vellumli.gui.lexicon.reloaded", (System.currentTimeMillis() - time)));
			}
		} else {
