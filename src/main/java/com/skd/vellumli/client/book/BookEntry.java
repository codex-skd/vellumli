...
import com.skd.vellumli.client.book.page.PageEmpty;
import com.skd.vellumli.client.book.page.PageQuest;
...
import com.skd.vellumli.common.util.ItemStackUtil.StackWrapper;
...
public final class BookEntry extends AbstractReadStateHolder implements Comparable<BookEntry> {
	...
	public boolean canAdd() {
		return (flag.isEmpty() || VellumliConfig.getConfigFlag(flag));
	}
		...
		if (extraRecipeMappings != null) {
			for (Entry<String, Integer> entry : extraRecipeMappings.entrySet()) {
				...
				try {
					...
					VellumliAPI.LOGGER.warn("Invalid extra recipe mapping: {} to page {} in entry {}: {}", key, pageNumber, id, e.getMessage());
					continue;
				}
				if (!stacks.isEmpty() && pageNumber < pages.length) {
					...
					VellumliAPI.LOGGER.warn("Invalid extra recipe mapping: {} to page {} in entry {}: Empty entry or page out of bounds", key, pageNumber, id);
				}
			}
		}
