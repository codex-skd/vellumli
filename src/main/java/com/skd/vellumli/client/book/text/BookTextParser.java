...
import com.skd.vellumli.client.book.gui.GuiBookCategory;
...
public class BookTextParser {
	...
	static {
		...
		register((parameter, state) -> {
			...
			if (result == null) {
				state.tooltip = Component.translatable("vellumli.gui.lexicon.keybind_missing", parameter);
				return "N/A";
			}

			state.tooltip = Component.translatable("vellumli.gui.lexicon.keybind", Component.translatable(result.getName()));
		...
		register((parameter, state) -> {
			...
			if (isExternal) {
				...
				state.tooltip = Component.translatable("vellumli.gui.lexicon.external_link");
				...
				if (entry != null) {
					state.tooltip = entry.isLocked()
							? Component.translatable("vellumli.gui.lexicon.locked").withStyle(ChatFormatting.GRAY)
							: entry.getName();
					int page = 0;
					if (anchor != null) {
	...
	public String expandMacros(@Nullable String text) {
		...
		if (i == expansionCap) {
			VellumliAPI.LOGGER.warn("Expanded macros for {} iterations without reaching fixpoint, stopping. " +
