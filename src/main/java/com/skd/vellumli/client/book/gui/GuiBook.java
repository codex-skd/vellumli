...
import com.skd.vellumli.client.base.PersistentData.Bookmark;
...
import com.skd.vellumli.client.book.gui.button.GuiButtonBookArrow;
import com.skd.vellumli.client.book.gui.button.GuiButtonBookBookmark;
import com.skd.vellumli.client.book.gui.button.GuiButtonBookMarkRead;
import com.skd.vellumli.common.base.VellumliSounds;
import com.skd.vellumli.common.book.Book;
import com.skd.vellumli.mixin.client.AccessorScreen;
...
public abstract class GuiBook extends Screen {
	...
	public void init() {
		...
		addRenderableWidget(new GuiButtonBook(this, width / 2 - 9, bookTop + FULL_HEIGHT - 5, 308, 0, 18, 9, this::canSeeBackButton, this::handleButtonBack,
				Component.translatable("vellumli.gui.lexicon.button.back"),
				Component.translatable("vellumli.gui.lexicon.button.back.info").withStyle(ChatFormatting.GRAY)));
	...
	public void addBookmarkButtons() {
		...
		// VELLUMLI NOTE (deferred in this beta): the multiblock visualization bookmark button
		// (Patchouli's MultiblockVisualizationHandler) is not ported yet.
		...
		if (tooltipStack != null) {
			...
			if (provider != null && (!(this instanceof GuiBookEntry) || ((GuiBookEntry) this).entry != provider.getFirst())) {
				Component t = Component.literal("(")
						.append(Component.translatable("vellumli.gui.lexicon.shift_for_recipe"))
						.append(")")
	...
	public boolean keyPressed(KeyEvent event) {
		...
		// VELLUMLI NOTE (deferred in this beta): JEI recipe keybind integration is not ported yet.
	...
	public void drawProgressBar(GuiGraphicsExtractor graphics, Book book, int mouseX, int mouseY, Predicate<BookEntry> filter) {
		...
		graphics.text(this.font, Component.translatable("vellumli.gui.lexicon.progress_meter"), barLeft, barTop - 9, book.headerColor, false);

		if (isMouseInRelativeRange(mouseX, mouseY, barLeft, barTop, barWidth, barHeight)) {
			List<Component> tooltip = new ArrayList<>();
			Component progressStr = Component.translatable("vellumli.gui.lexicon.progress_tooltip", unlockedEntries, totalEntries);
			tooltip.add(progressStr);

			if (unlockedSecretEntries > 0) {
				if (unlockedSecretEntries == 1) {
					tooltip.add(Component.translatable("vellumli.gui.lexicon.progress_tooltip.secret1").withStyle(ChatFormatting.GRAY));
				} else {
					tooltip.add(Component.translatable("vellumli.gui.lexicon.progress_tooltip.secret", unlockedSecretEntries).withStyle(ChatFormatting.GRAY));
				}
			}

			if (unlockedEntries != totalEntries) {
				tooltip.add(Component.translatable("vellumli.gui.lexicon.progress_tooltip.info").withStyle(ChatFormatting.GRAY));
			}
	...
	public static void playBookFlipSound(Book book) {
		if (ClientTicker.ticksInGame - lastSound > 6) {
			SoundEvent sfx = VellumliSounds.getSound(book.flipSound, VellumliSounds.BOOK_FLIP);
