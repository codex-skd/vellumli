...
public class GuiBookWriter extends GuiBook {
	...
	public void init() {
		...
		this.text = addRenderableOnly(new BookTextRenderer(this, Component.translatable("vellumli.gui.lexicon.editor.info"), LEFT_PAGE_X, TOP_PADDING + 20));
		...
		addRenderableWidget(new GuiButtonBook(this, bookLeft + 115, bookTop + PAGE_HEIGHT - 36, 330, 9, 11, 11, this::handleToggleHeaderButton, Component.translatable("vellumli.gui.lexicon.button.toggle_mock_header")));
		refreshText();
	}
	...
	void drawForegroundElements(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredStringNoShadow(graphics, I18n.get("vellumli.gui.lexicon.editor"), LEFT_PAGE_X + PAGE_WIDTH / 2, TOP_PADDING, book.headerColor);
		drawSeparator(graphics, book, LEFT_PAGE_X, TOP_PADDING + 12);

		if (drawHeader) {
			drawCenteredStringNoShadow(graphics, I18n.get("vellumli.gui.lexicon.editor.mock_header"), RIGHT_PAGE_X + PAGE_WIDTH / 2, TOP_PADDING, book.headerColor);
