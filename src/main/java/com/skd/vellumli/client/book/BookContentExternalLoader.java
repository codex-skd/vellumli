...
import com.skd.vellumli.common.book.BookFolderLoader;
...
/**
 * BookContentLoader that loads from the .minecraft/vellumli_books folder
 */
public final class BookContentExternalLoader implements BookContentLoader {
	...
	private ResourceLocation relativize(File root, File f) {
		...
		return ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, cleanPath);
	}
	...
	public LoadResult loadJson(Book book, ResourceLocation file) {
		try {
			...
			VellumliAPI.LOGGER.warn("Failed to load {}.", file, e);
			return null;
		}
