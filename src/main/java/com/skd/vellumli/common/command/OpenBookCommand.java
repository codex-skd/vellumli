package com.skd.vellumli.common.command;
...
public class OpenBookCommand {
	...
	public static void register(CommandDispatcher<CommandSourceStack> disp) {
		disp.register(Commands.literal("open-vellumli-book")
	...
	private static int doIt(Collection<ServerPlayer> players, Identifier book, @Nullable Identifier entry, int page) {
		for (ServerPlayer player : players) {
			if (entry != null) {
				VellumliAPI.get().openBookEntry(player, book, entry, page);
			} else {
				VellumliAPI.get().openBookGUI(player, book);
			}
		}
		return players.size();
	}
