...
public class VellumliConfig {
	private static final Map<String, Boolean> CONFIG_FLAGS = new ConcurrentHashMap<>();

	private static VellumliConfigAccess access = null;

	public static VellumliConfigAccess get() {
		return access;
	}

	public static void set(VellumliConfigAccess a) {
		if (access != null) {
	...
	public static boolean getConfigFlag(String name) {
		...
		if (b == null) {
			if (!name.startsWith("mod:")) {
				VellumliAPI.LOGGER.warn("Queried for unknown config flag: {}", name);
			}
			b = false;
		}
		return b == target;
