...
import com.skd.vellumli.client.book.*;
...
public class Book {
	...
	private static final ResourceLocation DEFAULT_MODEL = ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, "book_brown");
	private static final ResourceLocation DEFAULT_BOOK_TEXTURE = ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, "textures/gui/book_brown.png");
	private static final ResourceLocation DEFAULT_FILLER_TEXTURE = ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, "textures/gui/page_filler.png");
	private static final ResourceLocation DEFAULT_CRAFTING_TEXTURE = ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, "textures/gui/crafting.png");
	...
	@Nullable public final VellumliConfigAccess.TextOverflowMode overflowMode;
	...
	public Book(JsonObject root, XplatModContainer owner, ResourceLocation id, boolean external) {
		...
		this.landingText = GsonHelper.getAsString(root, "landing_text", "vellumli.gui.lexicon.landing_info");
		...
		this.openSound = SerializationUtil.getAsResourceLocation(root, "open_sound", VellumliSounds.BOOK_OPEN.getLocation());
		this.flipSound = SerializationUtil.getAsResourceLocation(root, "flip_sound", VellumliSounds.BOOK_FLIP.getLocation());
		...
		this.overflowMode = SerializationUtil.getAsEnum(root, "text_overflow_mode", VellumliConfigAccess.TextOverflowMode.class, null);
		...
		if (noBook) {
			// Need lazy parsing for mods that load after Vellumli, as parser looks up item and components
			...
			bookItem = Suppliers.memoize(() -> {
				try {
					...
					VellumliAPI.LOGGER.warn("Failed to parse item \"{}\" for book {} defined by mod {}, skipping",
	...
	public void reloadContents(Level level, boolean singleBook) {
		try {
			...
			VellumliAPI.LOGGER.error("Error loading and compiling book {}, using empty contents", id, e);
			contents = BookContents.empty(this, e);
		}
	}

	public final boolean advancementsEnabled() {
		return !VellumliConfig.get().disableAdvancementLocking()
				&& !VellumliConfig.get().noAdvancementBooks().contains(id.toString());
	}
	...
	public MutableComponent getSubtitle() {
		...
		try {
			...
			editionStr = Component.translatable("vellumli.gui.lexicon.dev_edition");
		}

		return Component.translatable("vellumli.gui.lexicon.edition_str", editionStr);
	}
