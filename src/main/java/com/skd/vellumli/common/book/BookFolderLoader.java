...
public class BookFolderLoader {
	...
	public static void findBooks() {
		...
		XplatModContainer self = IXplatAbstractions.INSTANCE.getModContainer(VellumliAPI.MOD_ID);
		File[] subdirs = loadDir.listFiles(File::isDirectory);
		if (subdirs == null) {
			VellumliAPI.LOGGER.warn("Failed to list external books in {}, not loading external books",
		...
		for (File dir : subdirs) {
			...
			try {
				res = ResourceLocation.fromNamespaceAndPath(VellumliAPI.MOD_ID, dir.getName());
			} catch (ResourceLocationException ex) {
				VellumliAPI.LOGGER.error("Invalid external book folder name {}, skipping", dir.getName(), ex);
				continue;
			}
			...
			try (FileInputStream stream = new FileInputStream(bookJson)) {
				...
				VellumliAPI.LOGGER.error("Failed to load external book json from {}, skipping", bookJson, e);
			}
		}
	}
